...
- **MoC nozzle**: `MoCNozzle` in `core/rocket/moc.py` solves a real minimum-length-nozzle characteristic net (centered Prandtl-Meyer fan → axis reflection → wall streamline), validated to ~0.1% against the isentropic area–Mach relation. The axisymmetric contour maps the validated planar area schedule to `r = rt·√(A/A*)`, giving the exact exit area ratio. A full axisymmetric characteristic-net march (with the radial source term) is a documented future refinement — its source-term integration is numerically delicate at high area ratios, so the exact area mapping is used instead.
